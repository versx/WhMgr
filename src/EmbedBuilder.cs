namespace WhMgr
{
    ...
    public class EmbedBuilder
    {
        ...
        public DiscordEmbed BuildPokemonMessage(PokemonData pokemon, string city, string pokemonImageUrl)
        {
            ...
            if (pokemon.IsMissingStats)
            {
                eb.Description = _lang.Translate("EMBED_POKEMON_TITLE_WITHOUT_DETAILS").FormatText(
                    pkmnName, form, pokemon.Gender.GetPokemonGenderIcon()) + "\r\n";
            ...
            else
            {
                eb.Description = _lang.Translate("EMBED_POKEMON_TITLE").FormatText(
                    pkmnName, form, pokemon.Gender.GetPokemonGenderIcon(), pokemon.IV, pokemon.Attack, pokemon.Defense, pokemon.Stamina, pokemon.Level) + "\r\n";
                eb.Description += _lang.Translate("EMBED_POKEMON_DESPAWN").FormatText
                    (pokemon.DespawnTime.ToLongTimeString(), pokemon.SecondsLeft.ToReadableString(true)) + (pokemon.DisappearTimeVerified ? "" : "~") + "\r\n";
            ...
            var matchesPvp = pokemon.MatchesGreatLeague || pokemon.MatchesUltraLeague;
            if (matchesPvp)
            {
                eb.Description += "\r\n**PvP Stats Ranking**\r\n";
            }
            ...
            {
                var greatLeague = Database.Instance.PvPGreat[pokemon.Id].Find(x =>
                {
                    if (int.TryParse(pokemon.Attack, out var atk) && atk == x.IVs.Attack &&
                        int.TryParse(pokemon.Defense, out var def) && def == x.IVs.Defense &&
                        int.TryParse(pokemon.Stamina, out var sta) && sta == x.IVs.Stamina)
                    {
                        return true;
                    }
                    return false;
                ...
                if (greatLeague != null)
                {
                    ...
                    eb.Description += $"**Rank:** #{greatLeague.Rank}\t**Level:** {greatLeague.Level}\t**CP:** {greatLeague.CP}\r\n";
                }
            }

            if (pokemon.MatchesUltraLeague)
            {
                var ultraLeague = Database.Instance.PvPUltra[pokemon.Id].Find(x =>
                {
                    if (int.TryParse(pokemon.Attack, out var atk) && atk == x.IVs.Attack &&
                        int.TryParse(pokemon.Defense, out var def) && def == x.IVs.Defense &&
                        int.TryParse(pokemon.Stamina, out var sta) && sta == x.IVs.Stamina)
                ...
                if (ultraLeague != null)
                {
                    ...
                    eb.Description += $"**Rank:** #{ultraLeague.Rank}\t**Level:** {ultraLeague.Level}\t**CP:** {ultraLeague.CP}\r\n";
                }
            }
        ...
        public static string GetPossibleInvasionEncounters(TeamRocketInvasion invasion)
        {
